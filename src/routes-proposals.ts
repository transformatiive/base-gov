  ...
  app.post('/api/announcements/:id/proposals/generate', { preHandler: [requireAuth, requirePlan('geracao_propostas')] }, async (req, reply) => {
    ...
    try {
      ...
      const buf = buildDocx({
        ...
        note: 'Rascunho gerado pelo BaseRadar. Requer revisão humana. A submissão no portal de contratação (BASE.gov / Vortal / acinGov / anoGov / SaphetyGov) é sempre manual.',
